  ...
  return (
    <div className="mt-6 rounded-3xl border border-white/10 bg-black/40 p-6">
      ...
      <div className="mt-2 text-xl font-semibold">
        Upgrade to unlock this full idea
      </div>
      <p className="mt-2 text-sm text-white/70">
        This page is available to paying members.
      </p>
...
export async function generateMetadata({
  ...
  const title = data?.idea_no
    ? `${titleCore} #${pad3(Number(data.idea_no))}`
    : titleCore;
  const description = (
    data?.summary ??
    data?.context ??
...
export default async function IdeaFullPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight">
            {i.ticker} <span className="text-white/40">•</span> Idea #
            {i.idea_no ? pad3(Number(i.idea_no)) : "—"}
          </h1>
          <div className="mt-2 text-xs text-white/40">
            {new Date(when as any).toLocaleString()}
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <span
            className={`rounded-full px-3 py-1 text-xs font-semibold ${
              i.direction === "long" || i.option_side === "call"
                ? "bg-emerald-500/15 text-emerald-400"
                : i.direction === "short" || i.option_side === "put"
